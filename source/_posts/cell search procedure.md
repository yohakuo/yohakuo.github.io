...
>摘抄自温金辉的博文 
## 概要
主要涉及PSS/SSS以及UE通过PSS/SSS能够得到哪些有用的信息。
...
## UE 的小区搜索状态
	...
	UE不仅需要在开机时进行小区搜索，为了支持移动性，UE会不停地搜索邻居小区、取得同步并估计该小区信号的接收质量，
	从而决定是否进行切换（handover，当UE处于RRC_CONNECTED态）或小区重选（cell re-selection，当UE处于RRC_IDLE态）。
